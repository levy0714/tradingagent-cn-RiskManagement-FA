...
def _search_web_bing(query: str, num: int = 8) -> list:
    """Bing 网页搜索兜底:返回 [(title, url), ...]。失败返回空列表。"""
    try:
        import urllib.parse
        ...
        html = resp.text
        items = _re.findall(r"<h2[^>]*>\s*<a[^>]*href=\"([^\"]+)\"[^>]*>(.*?)</a>", html, flags=_re.S)
        out = []
        for href, title in items:
            t = _re.sub(r"<[^>]+>", "", title)
            t = _strip_html_entities(t).strip()
            if not t:
                continue
            if "bing.com/ck/a" in href:
                m = _re.search(r"[?&]u=a1([^&]+)", href)
                if m:
...
def _cninfo_periodic_reports(code: str, start_date: str = None, end_date: str = None) -> str:
    """巨潮资讯定期报告公告搜索(必须显式传日期,否则接口默认 2023 年)。返回文本或空串。"""
    try:
        import akshare as ak
        sd = (start_date or "20230101").replace("-", "")
        ed = (end_date or "20300101").replace("-", "")
        df = ak.stock_zh_a_disclosure_report_cninfo(symbol=code, start_date=sd, end_date=ed)
        if df is None or len(df) == 0:
            return ""
        title_col = next((c for c in df.columns if "标题" in c or "名称" in c), df.columns[0])
        date_col = next((c for c in df.columns if "日期" in c), None)
        link_col = next((c for c in df.columns if "链接" in c or "URL" in c.upper() or "pdf" in c.lower()), None)
        kw = ("年度报告", "半年度报告", "季度报告", "业绩预告", "业绩快报", "主要经营数据")
        lines = []
        for _, row in df.iterrows():
            title = str(row[title_col])
            if any(k in title for k in kw):
                d = str(row[date_col]) if date_col else ""
                u = str(row[link_col]) if link_col and link_col in row else ""
                lines.append(f"- {d} | {title} | {u}")
        return "\n".join(lines[:30])
    except Exception:
        return ""


def _get_financial_statements_multi(code: str, report_type: str, freq: str, curr_date: str = None):
    """多源自动换源拉财报:新浪 → 东财(akshare) → 同花顺摘要(兜底)。
    返回 (df, source_name, errors)。全部失败时 df 为空,errors 含各源失败原因。"""
    errors = []
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            return df, "新浪", errors
        errors.append("新浪:空数据")
    except Exception as e:
        errors.append(f"新浪:{type(e).__name__}:{str(e)[:60]}")

    # 2) EastMoney via akshare (datacenter-web, 之前验证对 600010 可用)
    try:
        import akshare as ak
        prefix = "SH" if code.startswith(("6", "9")) else "SZ"
        em_symbol = prefix + code
        ...
        if df is not None and len(df) > 0:
            ...
            if "REPORT_DATE" in df.columns:
                ...
                if curr_date:
                    df = df[df["REPORT_DATE"] <= pd.to_datetime(curr_date)]
                df = df.sort_values("REPORT_DATE", ascending=False)
            return df.head(12), "东方财富", errors
        errors.append("东财:空数据")
    except Exception as e:
        errors.append(f"东财:{type(e).__name__}:{str(e)[:60]}")

    # 3) 同花顺财务摘要(兜底,覆盖营收/净利/EPS/ROE/毛利率)
    try:
        import akshare as ak
        df = ak.stock_financial_abstract_ths(symbol=code, indicator="按报告期")
        if df is not None and len(df) > 0:
            return df.tail(12), "同花顺摘要", errors
        errors.append("同花顺:空数据")
    except Exception as e:
        errors.append(f"同花顺:{type(e).__name__}:{str(e)[:60]}")

    # 4) 巨潮资讯定期报告公告(官方披露渠道)
    try:
        cn_txt = _cninfo_periodic_reports(code, "20230101", None)
        if cn_txt:
            return ("## 巨潮资讯定期报告公告(官方披露):\n" + cn_txt), "巨潮资讯", errors
        errors.append("巨潮:无定期报告公告")
    except Exception as e:
        errors.append(f"巨潮:{type(e).__name__}:{str(e)[:60]}")

    # 5) Bing 全网搜索兜底(自动发现官网/PDF/新闻稿)
    try:
        name = ""
        try:
            tq = _tencent_quote([code])
            if code in tq:
                name = tq[code].get("name", "")
        except Exception:
            pass
        if not name:
            name = f"{code} 股票"
        hits = _search_web_bing(f"{name} 年度报告 PDF")
        if not hits:
            hits = _search_web_bing(f"{name} 定期报告 公告")
        if hits:
            lines = [f"- {t} | {u}" for t, u in hits[:8]]
            return ("## 全网搜索到的财报线索(兜底,请人工核实官方原文):\n" + "\n".join(lines)), "全网搜索", errors
        errors.append("全网搜索:无结果")
    except Exception as e:
        errors.append(f"全网搜索:{type(e).__name__}:{str(e)[:60]}")
...
def get_balance_sheet(
    ...
    try:
        df, _fin_src, _fin_errs = _get_financial_statements_multi(code, "资产负债表", freq, curr_date)

        if isinstance(df, str):
            header = f"# 资产负债表 for {code} (A-stock, {freq}) - 数据源: {_fin_src}\n"
            header += f"# Data retrieved on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            return header + df
        if df.empty:
            detail = "; ".join(_fin_errs) if _fin_errs else "未知"
            return (
                f"No 资产负债表 data for A-stock '{code}' after full fallback chain "
                f"(新浪→东财→同花顺→巨潮→全网搜索). 各源失败原因: {detail}. "
                f"建议人工核实:上交所 www.sse.com.cn / 深交所 www.szse.cn / 巨潮资讯 www.cninfo.com.cn / 公司官网 定期报告专区。"
            )

        csv_string = df.to_csv(index=False)

        header = f"# Balance Sheet for {code} (A-stock, {freq})\n"
        header += "# Data source: " + _fin_src + " (multi-source fallback: 新浪→东财→同花顺)\n"
        header += (
            f"# Data retrieved on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        )
...
def get_cashflow(
    ...
    try:
        df, _fin_src, _fin_errs = _get_financial_statements_multi(code, "现金流量表", freq, curr_date)

        if isinstance(df, str):
            header = f"# 现金流量表 for {code} (A-stock, {freq}) - 数据源: {_fin_src}\n"
            header += f"# Data retrieved on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            return header + df
        if df.empty:
            detail = "; ".join(_fin_errs) if _fin_errs else "未知"
            return (
                f"No 现金流量表 data for A-stock '{code}' after full fallback chain "
                f"(新浪→东财→同花顺→巨潮→全网搜索). 各源失败原因: {detail}. "
                f"建议人工核实:上交所 www.sse.com.cn / 深交所 www.szse.cn / 巨潮资讯 www.cninfo.com.cn / 公司官网 定期报告专区。"
            )
...
def get_income_statement(
    ...
    try:
        df, _fin_src, _fin_errs = _get_financial_statements_multi(code, "利润表", freq, curr_date)

        if isinstance(df, str):
            header = f"# 利润表 for {code} (A-stock, {freq}) - 数据源: {_fin_src}\n"
            header += f"# Data retrieved on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            return header + df
        if df.empty:
            detail = "; ".join(_fin_errs) if _fin_errs else "未知"
            return (
                f"No 利润表 data for A-stock '{code}' after full fallback chain "
                f"(新浪→东财→同花顺→巨潮→全网搜索). 各源失败原因: {detail}. "
                f"建议人工核实:上交所 www.sse.com.cn / 深交所 www.szse.cn / 巨潮资讯 www.cninfo.com.cn / 公司官网 定期报告专区。"
            )
...
def _classify_news_source(url: str) -> str:
    """Classify a news URL into a credibility tier (official/authoritative/portal/self-media)."""
    if not url:
        return "未知"
    u = url.lower()
    if any(d in u for d in ("cninfo.com.cn", "sse.com.cn", "szse.cn", "csrc.gov.cn", ".gov.cn")):
        return "官方公告/监管"
    if any(d in u for d in ("cls.cn", "stcn.com", "cnstock.com", "cs.com.cn", "people.com.cn", "xinhuanet.com", "cctv.com", "gmw.cn", "yicai.com")):
        return "权威媒体"
    if any(d in u for d in ("k.sina.cn", "guba", "xueqiu", "mp.weixin", "zhihu", "tieba", "toutiao")):
        return "自媒体"
    if any(d in u for d in ("eastmoney.com", "finance.sina.com.cn", "sina.com.cn", "10jqka.com", "qq.com", "163.com", "hexun.com", "ifeng.com")):
        return "财经门户"
    return "其他"


def _fetch_ths_trade_events(code: str) -> str:
    """抓取同花顺移动版交易事件页(basic.10jqka.com.cn/mobile/<code>/trade.html)。
    提供业绩预告(含幅度)、业绩预披露日期、融资融券、公司公告列表。"""
    try:
        url = f"https://basic.10jqka.com.cn/mobile/{code}/trade.html"
        headers = {
            "User-Agent": (
        ...
        out = []
        for kw in ("业绩预告", "业绩预披露", "融资融券", "发布公告", "龙虎榜"):
            idx = text.find(kw)
            if idx != -1:
                seg = text[idx:idx + 350].strip()
                if seg:
                    out.append(seg)
        return "\n".join(out[:8]) if out else "(同花顺页面解析为空)"
    except Exception as e:
        return f"(同花顺交易事件页获取失败: {type(e).__name__}: {str(e)[:80]})"
...
def get_news(
    ...
    if "sina" in source_label.lower():
        news_str = "⚠ 以下新闻来自新浪(门户/自媒体混杂,可信度低,仅作极低参考;政策/公告结论以巨潮资讯与交易所官方为准)。\n\n"
    else:
        news_str = ""
    count = 0
    for art in articles:
        ...
        link = art.get("url", "")
        src_type = _classify_news_source(link)
        if src_type == "自媒体":
            warning = " [自媒体,仅情绪参考,不可作为政策/事实依据]"
        elif "sina" in source.lower():
            warning = " [新浪源,低可信,仅极低参考]"
        else:
            warning = ""
        news_str += f"### {title} (source: {source}{warning} | 类型:{src_type})\n"
        if content:
