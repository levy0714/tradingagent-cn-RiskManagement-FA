...
def resolve_stock_name(ticker: str) -> str | None:
    ...
    Prefers the Tencent A-share quote API (exchange-qualified) to avoid index-code
    collisions (e.g. 000932 is both 华菱钢铁 (A-share) and 中证主要消费指数).
    Falls back to the mootdx name-code map.
    """
    code = _resolve_display_code(ticker)
    if not re.match(r"^[036]\d{5}$", code):
        return None

    # 1) Tencent A-share quote first (exchange prefix avoids index collision)
    try:
        from tradingagents.dataflows.a_stock import _tencent_quote

        q = _tencent_quote([code])
        name = str(q.get(code, {}).get("name", "")).strip()
        if name:
            return _clean_stock_name(name)
    except Exception:
        pass

    # 2) mootdx name-code map fallback
    try:
